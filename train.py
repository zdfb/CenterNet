...
class train_centernet():
    def __init__(self):
        super(train_centernet, self).__init__()

        classes_path = 'model_data/voc_classes.txt'  # 类别存储路径
        ...
        self.input_shape = [512, 512]  # 输入图像尺寸 (h, w)
    ...
    def train(self, batch_size, learning_rate, start_epoch, end_epoch, Freeze=False):
        ...
        # 定义训练集与测试集
        train_dataset = CenternetDataset(self.train_lines, self.input_shape, self.num_classes, train=True)
        test_dataset = CenternetDataset(self.test_lines, self.input_shape, self.num_classes, train=False)
        ...
        for epoch in range(start_epoch, end_epoch):
            print('Epoch: ', epoch)
            train_loss, test_loss = fit_one_epoch(self.model, optimizer, train_data, test_data, device)
            lr_scheduler.step()

            # 若测试集loss小于当前极小值，保存当前模型
            if test_loss < self.loss_test_min:
                self.loss_test_min = test_loss
                torch.save(self.model.state_dict(), 'centernet.pth')
